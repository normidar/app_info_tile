...
.PHONY: rename
rename: ## Rename in all files from app_info_tile to <new_name>: `make rename <new_name>`
	@if [ -z "$(filter-out $@,$(MAKECMDGOALS))" ]; then \
		...
		echo "\033[0;32mRenaming app_info_tile to $$NEW_NAME in all files..."; \
		find . -type f \( -name "*.dart" -o -name "*.yaml" -o -name "*.yml" -o -name "*.md" -o -name "*.json" \) -not -path "./.dart_tool/*" -not -path "./build/*" -not -path "./.git/*" | xargs sed -i '' "s/app_info_tile/$$NEW_NAME/g"; \
		echo "\033[0;32mRenaming lib/app_info_tile.dart to lib/$$NEW_NAME.dart..."; \
		if [ -f "lib/app_info_tile.dart" ]; then \
			mv "lib/app_info_tile.dart" "lib/$$NEW_NAME.dart"; \
		fi; \
